...
#PYTHONPATH = "C:/Users/katsuya/OneDrive - OUMail (Osaka University)/Python/MD_LiNA_311_0424/:$PYTHONPATH"
...
class LiNA:
    ...
    def fit(self, X, G_sign, scale):
        ...
        ### array ver ###
        model_array = G_sign
        model_spec = ModelSpecificationParser.parse_model_specification_from_array(X, model_array)

        cfa = ConfirmatoryFactorAnalyzer(model_spec, disp=False, max_iter=100000)
        cfa.fit(X)        ## cfa.fit(X, G_sign)
        ...
        f = f_est
        if self._method == "nocurl":
            W_est = self._estimate_NoCurl(f) # check用
        elif self._method == "notears":
    ...
    def _estimate_NoCurl(self, f):
        method = "nocurl"
        bpr = BPR.BPR(l1=self._lambda1, l2=self._lambda2, loss_type=self._loss_type,
                      alpha1=self._alpha1, alpha2=self._alpha2)

        A, h, alpha, rho = bpr.fit(f, method)
        #G_est = nx.DiGraph(A)
        self._h = h
        self._rho = rho
    ...
    def _estimate_LiNA(self, f):
        ...
        def _func(w):
            ...
            obj = loss + 0.5 * rho * h * h + alpha * h + self._lambda1 * w.sum() + self._lambda2*(np.linalg.norm(W, ord = 'fro')**2) # add l2 penalty constraint
